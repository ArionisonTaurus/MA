%!TEX root = DieLoesungAllerMilleniumsprobleme.tex
	...
	\begin{definition}
		...
		\begin{itemize}
			\item Minus Eins (TODO: Minus groß oder klein?) genau dann, wenn $X$ leer ist
	...
	\begin{fact}
		Die Theorie T ist $\omega$-stabil genau dann, wenn jede Formel definierten Morleyrang hat
		TODO: Typen definierter Morleyrang, Interdefinierbarkeit, Addierbarkeit, partielle Isomorphismen
	\end{fact}
    ...
    \begin{corollary}\label{Isomorphismen linear disjunkt}
    	Seien im Folgenden die Körper $C,K,L,C',K',L'$ gegeben, sodass $K$ und $L$ linear disjunkt über $C$ sind sowie $K'$ und $L'$ über $C'$. Es existiere ein Isomorphismus $C\cong C'$, der eine Fortsetzung auf $K\cong K'$ und eine andere auf $L\cong L'$ habe. Dann gibt es eine \glqq{}fusionierte\grqq{} Fortsetzung auf $K[L]\cong K'[L']$, denn die zwei Fortsetzungen induzieren einen Isomorphismus $$K\otimes_CL\cong K'\otimes_C'L'.$$ 
    \end{corollary}
    
    \begin{remark}
    	Die vorige Folgerung ist das algebraische Analogon zu folgender Aussage aus der Stabilitätstheorie: Sei $\fM,\fN$ zwei Modelle zu irgendeiner $\omega$-stabilen vollständigen, abzählbaren Theorie und Mengen $S\subseteq M,S'\subseteq N$, sodass es eine elementare Abbildung $$\varphi:S\cong S'$$ gibt, außerdem seien $a,b$ Tupel in $M$ und $a',b'$ Tupel in $N$, sodass gelte: $$\varphi(\tp(a/S))=\tp(a'/S'),\varphi(\tp(b/S))=\tp(b'/S).$$
    	...
    	An diesem Punkt ist die Korrespondenz zwischen diesen beiden Formulierungen noch nicht in Gänze zu verstehen, dafür wird unter anderem noch die Beziehung zwischen linearer und algebraischer Disjunktheit näher untersucht werden müssen. Es sei aber schon einmal darauf hingewiesen, dass die Stationarität von $\tp(b'/S')$ gerade die Eigenschaft darstellt, dass eine algebraische Abhängigkeit zwischen Elementen von $b'$ über $S'$ eine lineare Abhängigkeit dieser Elemente von $b'$ über $S'$ impliziert.
    \end{remark}
    ...
    \begin{remark}
    	Auch hier kann man wieder Vergleiche zur Stabilitätstheorie herstellen: Die Körpererweiterung $K\subseteq K(a)$ ist für ein transzendentes $a$ ohnehin immer regulär, da dann die Basis $1,a,a^2,\dots$ von $K[a]$ über $K$ auch über $\overline{K}$ linear unabhängig bleibt. Es ist aber auch $\tp(a/K)$ stationär, weil der transzendente Typ in streng minimalen Theorien eindeutig ist.\\
    	Auch wenn $a$ algebraisch über $K$ ist, gilt die Äquivalenz von regulärer Körpererweiterung und stationärem Typ: Es ist $\tp(a/K)$ genau dann stationär, wenn $a$ in $K$ ist. Aber das gilt genau dann, wenn die Erweiterung regulär ist, denn das ist das selbe wie die lineare Unabhängigkeit von $1$ und $a$ über $K$ auf $\overline{K}$ zu übertragen.
    \end{remark}
