...
Um definierbare Funktionen besser zu verstehen, ist es notwendig, sich mit dem definierbaren Abschluss in $\lingua^P$ zu beschäftigen. Damit kann man zeigen, dass in einer Variablen definierbare Funktionen \glqq{}fast überall\grqq{} $\lingua$-definierbar sind. Wir erinnern uns, dass ein dichtes Paar $(B,A)$ fixiert war und eine spezielle Menge $S\subseteq B$. Mit \glqq{}definierbar abgeschlossen\grqq{} ist im Folgenden \glqq{}$\lingua^P$-definierbar abgeschlossen\grqq{} gemeint. Aus dem bekannten Back\&Forth-System erhält man eine einfache Möglichkeit, definierbar abgeschlossene Mengen zu finden.

\begin{lemma}\label{Freie Definierbarkeit}
	Sei $(D,C)\subseteq(B,A)$ frei, dann ist $D$ definierbar abgeschlossen in $(B,A)$.
\end{lemma}
\begin{proof}
	Für eine beliebige Struktur $(B',A')\succeq (B,A)$ gelten die Voraussetzungen ebenso, da $\acl$-Abhängigkeit von Elementen aus $D$ über $A$ als Teil von Typen äquivalent ist zu $\acl$-Abhängigkeit über $A'$. Ebenso ist $D$ genau dann definierbar abgeschlossen in $(B,A)$, wenn es in $(B',A')$ definierbar abgeschlossen ist. Also sei $(B,A)$ jetzt schon ohne Einschränkungen hinreichend saturiert und $b$ in $B\setminus D$.\\
	...
	So kann man den Isomorphismus auf $Da$ fortsetzen mit Bild $Da'$; da aber $a$ und $a'$ per Konstruktion unabhängig über $D$ waren, sind auch $Da$ und $Da'$ unabhängig über $D$, also gilt $Da\cap Da'=D$ und das Bild von $b$ kann nicht $b$ selbst sein. Also gibt es auch in diesem Fall mehrere Möglichkeiten für eine Fortsetzung, daher kann $b$ nicht definierbar über $D$ sein.
\end{proof}

Das vorige Lemma kann man unter bestimmten Bedingungen umkehren, so zum Beispiel für $\operatorname{RCF^d}$.
\begin{definition}
	Die Theorie $\td$ sei \textbf{zahm für Paare}, wenn jede definierbar abgeschlossene Menge in jedem Modell $(B,A)\models\td$ speziell ist.
\end{definition}
\newpage
\begin{lemma}\label{RCF zahm}
	Die Theorie $\operatorname{RCF^d}$ ist zahm für Paare.
\end{lemma}
\begin{proof}
	Sei $(B,A)$ ein Modell und $S$ definierbar abgeschlossen. Dann ist $S$ Träger einer $\lingua$-Unterstruktur von $B$, also auch Modell von RCF. Sei $\overline{B}$ ein algebraischer Abschluss von $B$, dann stimmt der modelltheoretische algebraische Abschluss bzgl. RCF auf $B$ mit dem auf $B$ eingeschränkten algebraischen Abschluss bzgl. ACF auf $\overline{B}$ überein, wir brauchen sie also nicht zu unterscheiden. Zu zeigen ist dann $$S\ad_{S\cap A}A.$$
...
\begin{lemma}
	Gegeben sei eine $\lingua^P_S$-definierbare Funktion $F:A^n\rightarrow A$. Dann gibt es $S\cap A$-definierbare $f_1,\dots,f_k:A^n\rightarrow A$ in $A$, sodass für alle $a$ in $A^n$ ein $f_i$ existiert mit $F(a)=f_i(a)$.
\end{lemma}
\begin{proof}
	...
	Wenn das Lemma nicht gälte, gilt für alle $k$ und alle in $A$ über $S\cap A$ definierbaren $f_1,\dots,f_k:A^n\rightarrow A$, dass ein $a$ aus $A$ existiert mit $f_i(a)\neq F(a)$ für alle $i$. Also ist der partielle Typ $$\{P(x)\}\cup\{F(x)\neq f(x)\mid f:A^n\rightarrow A\text{ definierbar in }\lingua_{S\cap A}\}$$ konsistent und es existiert eine Oberstruktur $(B,A)\preceq(B',A')$ und ein Tupel $a'$ in $A'^n$ mit $F(a')\neq f(a')$ für alle $\lingua_{S\cap A}$-definierbaren $f:A'^n\rightarrow A'$.\newpage
	Allerdings ist die Inklusion $$(S,S\cap A)\subseteq (B,A)$$ frei, wegen Elementarität der Erweiterung auch $$(S,S\cap A)\subseteq (B',A').$$ Nach Lemma \ref{Unabhängigkeitsregeln} (6.) ist, weil $a'$ in $A'^n$ liegt, die Inklusion $$(Sa',(S\cap A)a')\subseteq(B',A')$$ frei und nach Lemma \ref{Freie Definierbarkeit} ist $Sa'$ also $\lingua^P$-definierbar abgeschlossen. Da $F(a')$ aber $\lingua^P$-definierbar über $Sa'$ ist, liegt es in $Sa'$, außerdem bildet $F$ nach $A'$ ab. Wegen Unabhängigkeit gilt also $$F(a')\in Sa'\cap A'=(S\cap A)a'$$ und es gibt eine $\lingua_{S\cap A}$-definierbare Abbildung $f:A'^n\rightarrow A'$ mit $f(a')=F(a')$ - ein Widerspruch!
\end{proof}

\begin{lemma}
	Es sei $F:B\rightarrow B$ eine $\lingua^P_S$-definierbare Funktion. Dann gibt es\linebreak $\lingua_S$-definierbare $f_1,\dots,f_k:B\rightarrow B$ und eine kleine $\lingua^P_S$-definierbare Menge $X\subseteq B$, sodass für alle $b$ aus $B\setminus X$ ein $f_i$ existiert mit $F(b)=f_i(b)$.
\end{lemma}
\begin{proof}
	Ohne Einschränkungen ist wieder $S$ ein Modell.\\
	Wenn die Aussage nicht gälte, gilt für alle $k$, alle kleinen $\lingua^P_S$-definierbaren Mengen ${X\subseteq B}$ und alle $\lingua_S$-definierbaren $f_1,\dots,f_k:B\rightarrow B$, dass ein $b$ aus $B\setminus X$ existiert mit $f_i(b)\neq F(b)$ für alle $i$.\\
	Also ist der partielle Typ $$\{x\notin X\mid X\text{ klein, }\lingua^P_S\text{-definierbar}\}\cup\{F(x)\neq f(x)\mid f:B\rightarrow B\text{ definierbar in }\lingua_S\}$$ (endlich) konsistent und es existiert eine Oberstruktur $(B,A)\preceq(B',A')$ und ein $b$ in $$B'\setminus\bigcup\limits_{f:B'^n\rightarrow B'\text{ definierbar in }\lingua_S}f(A'^n)=B'\setminus A'S$$ mit $F(b')\neq f(b')$ für alle $\lingua_S$-definierbaren $f:B'\rightarrow B'$.\newpage
	Allerdings ist die Inklusion $$(S,S\cap A)\subseteq(B',A')$$ frei und nach Lemma \ref{Unabhängigkeitsregeln} (7.) \--- weil $b'$ nicht in $A'S$ liegt \--- die Inklusion $$(Sb',S\cap A)\subseteq(B',A')$$ ebenso. Nach Lemma \ref{Freie Definierbarkeit} ist $Sb'$ also $\lingua^P$-definierbar abgeschlossen. Da $F(b')$ definierbar in $\lingua^P_{Sb'}$ ist, ist es in $Sb'$. Also existiert eine $\lingua_S$-definierbare Abbildung $f:B'\rightarrow B'$ mit $f(b')=F(b')$ \--- ein Widerspruch!
\end{proof}

\begin{theorem}\label{Satz 3}
	Gegeben sei eine $\lingua^P_S$-definierbare Funktion $F:B\rightarrow B$. Dann stimmt $F$ auf bis auf eine $\lingua^P_S$-definierbare kleine Menge mit einer $\lingua_S$-definierbaren Funktion überein.
\end{theorem}
\begin{proof}
	Nach dem vorigen Lemma existieren $\lingua_S$-definierbare $f_1,\dots,f_k:B\rightarrow B$ und ein kleines $\lingua^P_S$-definierbares $X$, sodass für alle $b$ außerhalb von $X$ ein $i$ existiert mit $F(b)=f_i(b)$. Wenn $k=1$ ist, ist das die gewünschte Aussage, wenn nicht, zeige dass man $k$ weiter reduzieren kann. Dafür partitionieren wir $B$ mit Satz \ref{Satz 4} in\linebreak $\lingua_S$-definierbare ${E,T,D,K,C\subseteq B}$, sodass $E$ endlich ist, alle anderen Mengen dafür offen, sowie
	$$T\subseteq\{F=f_1\}\text{ und }D\cap\{F=f_1\}=\emptyset\text{ gilt,}$$ $$K':=K\cap\{F=f_1\}\text{ klein, dicht und kodicht in }K\text{ ist}$$ $$\text{sowie }C':=C\cap\{F=f_1\}\text{ koklein, dicht und kodicht in }C\text{ ist}.$$
	...
	\end{itemize}
    Also nimmt $F$ auf $B\setminus X'$ immer die Werte von $f,f_3,\dots,f_k$ an und mit einer Induktion ist die Aussage gezeigt.
\end{proof}
\newpage
\begin{lemma}\label{Stückweise stetige Abbildungen}
	Sei $f:B\rightarrow B$ eine stückweise stetige und $\lingua^P_S$-definierbare Funktion. Dann ist $f$ schon $\lingua_S$-definierbar.
\end{lemma}
\begin{proof}
	Nach dem vorigen Satz stimmt $f$ bis auf eine kleine $\lingua^P_S$-definierbare Menge $X$ mit einer $\lingua_S$-definierbaren Funktion $f'$ überein. Wegen der o-Minimalität von T ist $f'$ auch stückweise stetig. Unterteile $X$ wie in Satz \ref{Satz 4} und verfeinere die Unterteilung so, dass $f$ und $f'$ auf jedem daraus resultierenden Intervall stetig sind. Für jedes Intervall $I$ dieser Unterteilung gilt dann entweder, dass $X\cap I$ dicht und kodicht in $I$ ist oder, dass $X\cap I=\emptyset$. Der erste Fall kann nie eintreten, da zwei stetige Funktionen, die auf der dichten Teilmenge $I\setminus X$ übereinstimmen, schon auf ganz $I$ übereinstimmen. Also ist $X$ endlich und $f$ kann mit $\lingua$ definiert werden (nämlich durch $f'$ außerhalb von $X$ und ansonsten manuell). Da diese Unterteilung $\lingua_S$-definierbar ist, ist auch $f$ schon $\lingua_S$-definierbar.
\end{proof}

\section{Offene und abgeschlossene $\lingua^P$-definierbare Mengen}
Nach der Beschreibung der Struktur von $\lingua^P$-definierbaren Teilmengen von $B$ liegt die Vermutung nahe, dass offene und abgeschlossene $\lingua^P$-definierbare Mengen schon\linebreak $\lingua$-definierbar sind.\\
Dies lässt sich in vielen Situationen auch beweisen. Im Folgenden werden mehrere Möglichkeiten dafür angegeben. Hierbei sei $(B,A)$ wieder ein echtes dichtes Paar und $S$ speziell.\\
...
\begin{lemma}\label{Offenheit einfache Fälle}\ 
	\begin{itemize}
		\item Sei eine Menge $Z\subseteq B$ gegeben, die offen und $\lingua^P_S$-definierbar ist. Dann ist $Z$ schon $\lingua_S$-definierbar.
...
\begin{proof}\ 
	\begin{itemize}
		\item In der Darstellung von Satz \ref{Satz 4} kann der Fall $Z\cap I$ dicht und kodicht nicht auftreten. Die Menge $Z\cap I$ ist nämlich offen und kann daher nicht kodicht in $I$ sein (denn sonst würde sie ein Element ihres eigenen Komplements enthalten).\newpage
		Also sind die $\lingua^P_S$-definierbaren offenen Teilmengen von $B$ gerade die endlichen Vereinigungen von Intervallen mit Rand aus $\dcl(S)\cup\{\pm\infty\}$ und Punkten aus $\dcl(S)$. Das ist $\lingua_S$-definierbar (und Punkte benutzt man dabei auch nur, wenn man ein Intervall \glqq{}unnötig\grqq{} unterteilen will).
		\item Wenn $Z\subseteq B^n$ abgeschlossen und $\lingua^P_S$-definierbar ist, ist $Z^c$ offen und\linebreak $\lingua^P_S$-definierbar. Also ist $Z^c$ $\lingua_S$-definierbar per Voraussetzung, damit ist $Z$ dann ebenso $\lingua_S$-definierbar.
	\end{itemize}
\end{proof}


Für mehrstellige Mengen kann man folgenden Fakt aus \cite{Piz} benutzen, die Voraussetzungen dafür sind allerdings nicht zwangsläufig erfüllt. Für einige Theorien wie z.B. $\operatorname{RCF^d}$ ist das aber ein gangbarer Weg (siehe auch Lemma \ref{RCF zahm}).
\begin{fact}\label{Kurven}
	Es sei $F:B^n\rightarrow B$ eine $\lingua^P_S$-definierbare Funktion und $\td$ sei zahm für Paare. Dann ist $F$ genau dann definierbar in $\lingua_S$, wenn für alle speziellen Obermengen $X\supseteq S$ und alle $\lingua_X$-definierbaren partiellen Funktionen $\alpha: Y\rightarrow B^n$ für eine offene Teilmenge $Y$ von $B$ auch die Verkettung $F\circ\alpha$ schon $\lingua_X$-definierbar ist.
\end{fact}
...
\end{theorem}
\begin{proof}
	Es reicht, das für offene Mengen zu zeigen; genauer reicht es schon, die Definierbarkeit für charakteristische Funktionen solcher Mengen zu zeigen. Sei $Z\subseteq B^n$ eine offene $\lingua^P_S$-definierbare Menge und $X\supseteq S$ speziell, $\alpha:Y\rightarrow B^n$ eine $\lingua_X$-definierbare, partielle Funktion und $Y\subseteq B$ offen. Zu zeigen ist für die Anwendung von Fakt \ref{Kurven}, dass $\chi_Z\circ\alpha$ definierbar in $\lingua_X$ ist.\\
	Man kann annehmen, dass $\alpha$ stetig und $Y$ ein Intervall ist, sonst zerlege $Y$ in Intervalle, auf denen $\alpha$ stetig ist, und Punkte. Das ändert nämlich nichts an irgendwelchen Definierbarkeiten und im Fall, dass $Y$ eine einelementige Menge ist, ist $\chi_Z\circ\alpha$ entweder konstant $0$ oder konstant $1$. Es ist festzustellen, dass $$1=\chi_Z\circ\alpha(x)\Leftrightarrow\alpha(x)\in Z\Leftrightarrow x\in\alpha^{-1}(Z)$$ gilt.\newpage
	Da aber $\alpha^{-1}(Z)$ eine $\lingua^P_X$-definierbare Teilmenge von $B$ ist, offen in $B$ wegen der Stetigkeit von $\alpha$ und der Offenheit von $Z$ sowie $Y$ und zusätzlich $X$ speziell ist, ist $\alpha^{-1}(Z)$ nach Lemma \ref{Offenheit einfache Fälle} $\lingua_X$-definierbar. Also ist $\chi_Z\circ\alpha$ definierbar in $\lingua_X$ durch $$\chi_Z\circ\alpha(x)=\left\{\begin{array}{ll}
	1&x\in\alpha^{-1}(Z)\\
	0&\text{sonst}
	\end{array}\right..$$
\end{proof}
...
Es gibt auch andere Möglichkeiten, die gesuchte Aussage für manche Theorien zu beweisen. Für Teilmengen von $\setR^n$ in einer o-minimalen Erweiterung von RCF mit Grundmenge $\setR$ wird das zum Beispiel in \cite{VanDenDries} beschrieben. Aber auch in \cite{Hieronymi} gibt es eine weitere Lösungsmöglichkeit dieses Problems.