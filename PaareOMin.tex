...
\end{theorem}
\begin{proof}
	Sei $f:B^n\rightarrow B$ eine durch $\varphi(x,y,b)$ definierbare Abbildung mit $\varphi$ einer\linebreak$\lingua_A$-Formel und $b\in B^m$ für ein $m\in\setN$ definiert. Für $\dim(b/A)=0$ ist $f(A^n)\subseteq A$ klar, deswegen sei ohne Einschränkungen $\dim(b/A)\geq1$. Definiere
	\begin{align*}
...
\end{lemma}
\begin{proof}
	Schreibe $Y=\phi(b,\fM)$ für ein $b\in M^m$ und definiere $Z:=\phi(\fM)$. Wenn man $Z$ in 0-definierbare Zellen $(Z_i)_i$ zerlegt, erhält man $Y$ als endliche Vereinigung der Fasern $((Z_i)_b)_i$.  Es sei also ohne Einschränkungen $Z$ schon eine 0-definierbare Zelle.\\
